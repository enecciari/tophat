    ...
    _buildCPUDetailRows() {
        ...
        this._findTempMonitors().then(hasTemp => {
            new FileModule.File('/proc/cpuinfo').read().then(lines => {
                ...
                log(`[${this.name}] Error reading /proc/cpuinfo: ${err}`);
                this.hasProc = false;
            });
        }).catch(err => {
            log(`[${this.name}] Error finding temperature monitors: ${err}`);
            this.hasTemp = false;
        });
    }
    ...
    _findTempMonitors() {
        return new Promise((resolve, reject) => {
            ...
            new FileModule.File(basePath).list().then(files => {
                ...
                log(`[${this.name}] Error listing files in ${basePath}: ${err}`);
                reject(err);
            });
        });
    }
    ...
    _readCPUTemps() {
        this.cpuTempMonitors.forEach((path, id) => {
            new FileModule.File(path).read().then(temp => {
                ...
                log(`[${this.name}] Error reading ${path}: ${err}`);
                this.hasTemp = false;
            });
        });
    }
